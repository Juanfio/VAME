...
def test_train_model_losses_files_exists(setup_project_and_train_model):
    ...
    # save logged losses
    train_losses_path = (
        Path(project_path) / "model" / "model_losses" / f"train_losses_{model_name}.npy"
    )
    test_losses_path = (
        Path(project_path) / "model" / "model_losses" / f"test_losses_{model_name}.npy"
    )
    kmeans_losses_path = (
        Path(project_path)
        / "model"
        / "model_losses"
        / f"kmeans_losses_{model_name}.npy"
    )
    kl_losses_path = (
        Path(project_path) / "model" / "model_losses" / f"kl_losses_{model_name}.npy"
    )
    weight_values_path = (
        Path(project_path)
        / "model"
        / "model_losses"
        / f"weight_values_{model_name}.npy"
    )
    mse_train_losses_path = (
        Path(project_path)
        / "model"
        / "model_losses"
        / f"mse_train_losses_{model_name}.npy"
    )
    mse_test_losses_path = (
        Path(project_path)
        / "model"
        / "model_losses"
        / f"mse_test_losses_{model_name}.npy"
    )
    fut_losses_path = (
        Path(project_path) / "model" / "model_losses" / f"fut_losses_{model_name}.npy"
    )
...
def test_evaluate_model_images_exists(setup_project_and_evaluate_model):
    ...
    reconstruction_image_path = (
        Path(project_path) / "model" / "evaluate" / "Future_Reconstruction.png"
    )
    loss_image_path = (
        Path(project_path) / "model" / "evaluate" / f"MSE-and-KL-Loss{model_name}.png"
    )
